...
# License: LGPL v 2.1
# Autor: Lucio Gomez (psicofil@gmail.com) and Alessio Nava (a.nava@duplomatic.com)
...
class Contact3D(QWidget):
    def __init__(self):
        super(Contact3D, self).__init__()
        self.initUI()
        # self.selectParts()
    ...
    def selectParts(self):
        try:
            ...
            QMessageBox.critical(
                None, 'Error', "Error in selecting parts.", QMessageBox.Abort)
    ...
    def proceed(self):
        ...
        try:
            ...
            QMessageBox.critical(
                None, 'Error', "Select 2 or more 3D parts first.", QMessageBox.Abort)
        num_cont = 0
                ...
                if i != j and i < j:
                    try:
                        isOk, res1, res2 = geompy.FastIntersect(
                            selobj[i], selobj[j], gap)
                    except:
                        QMessageBox.critical(
                            None, 'Error', "Error in FastIntersect operation.", QMessageBox.Abort)
                    if isOk > 0:
                        N_C = len(res1)
                        N_C2 = len(res2)
                                ...
                                if Common == False:
                                    if area_com > 0.0:
                                        ...
                                        geompy.addToStudyInFather(
                                            selobj[i], CONT[h], name_group_i)
                                        name_group_j = 'CZ_' + \
                                            str(j) + str(i) + '_' + str(h)
                                        geompy.addToStudyInFather(
                                            selobj[j], CONT2[k], name_group_j)
                                        num_cont += 1
                                if Common == True:
                                    if area_com > 0.0:
